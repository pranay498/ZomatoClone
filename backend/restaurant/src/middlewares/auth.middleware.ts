...
export const requireSeller = (req: Request, res: Response, next: NextFunction) => {
  console.log("🟡 [Seller Middleware] requireSeller called");
  console.log("🟡 [Seller Middleware] User role:", req.userRole);
  
  if (!req.userRole) {
    console.error("🔴 [Seller Middleware] User role not found");
    return next(new AppError("User role not found. Please authenticate first", 401));
  }

  if (req.userRole !== "seller") {
    console.error("🔴 [Seller Middleware] User is not a seller, role is:", req.userRole);
    return next(new AppError("Access denied. Only sellers can access this resource", 403));
  }

  console.log("🟢 [Seller Middleware] User is seller, allowing access");
  next();
};
